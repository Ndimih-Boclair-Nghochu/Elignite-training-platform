...
export function Footer() {
  const [settings, setSettings] = useState<Settings>({
    schoolName: "ELIGNITE",
    schoolLogoUrl: null,
    schoolAddress: "Bamenda, Cameroon",
    schoolEmail: "admissions@elignite.cm",
    schoolPhone: "+237 677 000 111",
  });
  ...
  useEffect(() => {
    fetch("/api/public/settings")
      ...
      .then((d) => {
        if (d) {
          setSettings({
            schoolName: d.schoolName || "ELIGNITE",
            schoolLogoUrl: d.schoolLogoUrl || null,
            schoolAddress: d.schoolAddress || "Bamenda, Cameroon",
            schoolEmail: d.schoolEmail || "admissions@elignite.cm",
            schoolPhone: d.schoolPhone || "+237 677 000 111",
          });
        }
      })
      .catch(() => {});
  ...
  return (
    <footer className="border-t border-blue-200 bg-[#0d5bd7] text-white">
      <div className="mx-auto grid max-w-7xl gap-10 px-4 py-16 sm:px-6 lg:grid-cols-[1.2fr_0.8fr_0.8fr_1fr] lg:px-8">
        <div className="space-y-4">
          <div className="flex items-center gap-3">
            {settings.schoolLogoUrl && (
              <div className="h-10 w-10 rounded-lg bg-white overflow-hidden flex items-center justify-center shrink-0">
                <img src={settings.schoolLogoUrl} alt={`${settings.schoolName} logo`} className="h-full w-full object-contain p-1" />
              </div>
            )}
            <p className="text-sm font-semibold uppercase tracking-[0.25em] text-blue-100">{settings.schoolName}</p>
          </div>
          ...
          <p className="max-w-md text-sm leading-7 text-blue-100/85">
            {settings.schoolName} Training Platform helps learners build practical digital skills through structured programs, expert support, and hands-on projects.
          </p>
        </div>
      ...
      <div className="border-t border-white/15 px-4 py-5 text-center text-xs text-blue-100/80 sm:px-6 lg:px-8">
        Copyright 2026 {settings.schoolName} Training Platform. Practical digital learning for careers in tech.
      </div>
    </footer>
  );
}
