  ...
  try {
    ...
    const teachers = await prisma.teacher.findMany({
      include: {
        ...
        teacherPrograms: { include: { program: { select: { title: true, slug: true } } } },
      },
      orderBy: { id: "asc" },
    });
    ...
    teachers.forEach((teacher, index) => {
      const fullName = `${teacher.user.firstName || ""} ${teacher.user.lastName || ""}`.trim() || "Unnamed staff";
      const programs = teacher.teacherPrograms.map((tp) => tp.program.title || tp.program.slug).join(", ");
      ensurePdfSpace(doc, 92);
      doc.roundedRect(40, doc.y, 515, 84, 10).fillAndStroke("#ffffff", "#dbeafe");
      const top = doc.y + 12;
      doc.font("Helvetica-Bold").fontSize(11).fillColor("#0f172a").text(`${index + 1}. ${fullName}`, 54, top);
    ...
    console.error("Teacher report export error:", error);
