...
export default function CeoStaffPage() {
  ...
  const filtered = staff.filter((t) =>
    `${t.user.firstName} ${t.user.lastName} ${t.user.email} ${t.occupation || ""} ${t.profession || ""} ${t.matricle || ""}`
      .toLowerCase()
  ...
  async function handleCreateStaff(e: React.FormEvent) {
    e.preventDefault();
    if (!form.firstName || !form.lastName || !form.email || !form.occupation || !form.department) {
            ...
            <form onSubmit={handleCreateStaff} className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-1">
                  ...
                  <Input value={form.occupation} onChange={(e) => setForm((p) => ({ ...p, occupation: e.target.value }))} required />
                </div>
                <div className="space-y-1">
              ...
              <div className="space-y-2">
                ...
                {programs.length === 0 ? (
                  <p className="text-sm text-gray-400">No programs available yet.</p>
                ) : (
                ...
                {selectedProgramIds.length > 0 && (
                  <p className="text-xs text-gray-500">{selectedProgramIds.length} program(s) selected</p>
                )}
              </div>

              <div className="flex items-center justify-end gap-2">
                <Button type="button" variant="secondary" onClick={() => { setOpen(false); resetForm(); }} disabled={creating}>
                  Cancel
                </Button>
                <Button type="submit" disabled={creating}>
                  {creating ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" />Saving...</> : "Create Staff"}
                </Button>
              </div>
            </form>
          </DialogContent>
                    ...
                    <TableRow key={t.id}>
                      ...
                      <TableCell>
                        <div className="text-sm space-y-0.5">
                          ...
                          <div className="flex items-center gap-1 text-gray-500"><Phone className="h-3 w-3" />{t.user.phone || "–"}</div>
                        </div>
                      </TableCell>
                      ...
                      <TableCell>
                        <div className="flex flex-wrap gap-1">
                          {t.programs && t.programs.length > 0 ? (
                            t.programs.map((prog) => (
                              <Badge key={prog.id} variant="outline" className="text-xs font-mono">
                                {prog.programCode}
                              </Badge>
                            ))
                          ) : (
